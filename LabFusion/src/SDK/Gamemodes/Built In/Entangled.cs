...
public class Entangled : Gamemode
{
    ...
    protected PlayerID GetPartner(PlayerID id)
    {
        ...
        {
            return PlayerIDManager.GetPlayerID(value);
        }
    ...
    protected override void OnMetadataChanged(string key, string value)
    {
        ...
        if (GetPartnerKey(PlayerIDManager.LocalID) == key)
        {
            ...
            {
                OnReceivePartner(PlayerIDManager.GetPlayerID(value));
            }
        }
        else if (!string.IsNullOrEmpty(value) && key.StartsWith(PlayerPartnerKey))
        {
            var id = GetPlayerId(key);

            if (!string.IsNullOrEmpty(id))
            {
                _tethers.Add(new EntangledTether(id, PlayerIDManager.GetPlayerID(value)));
            }
        }
    }
