...
public class Entangled : Gamemode
{
    public class EntangledTether
    {
        ...
        public void OnUpdate()
        {
            ...
            if (selfPelvis == null && NetworkPlayerManager.TryGetPlayer(player1.SmallID, out var netPlayer1) && netPlayer1.NetworkRig.HasRig)
            {
                selfPelvis = netPlayer1.NetworkRig.RigRefs.RigManager.physicsRig.torso._pelvisRb;
            }

            if (otherPelvis == null && NetworkPlayerManager.TryGetPlayer(player2.SmallID, out var netPlayer2) && netPlayer2.NetworkRig.HasRig)
            {
    ...
    protected void AssignPartners(PlayerID player1, PlayerID player2)
    {
        ...
        if (NetworkPlayerManager.TryGetPlayer(player2.SmallID, out var rep) && rep.NetworkRig.HasRig)
        {
            PlayerSender.SendPlayerTeleport(player1.SmallID, rep.NetworkRig.RigRefs.RigManager.physicsRig._feetRb.position);
        }
    }
    ...
    {
        if (Metadata.TryGetMetadata(GetPartnerKey(id), out var value))
        {
            return PlayerIDManager.GetPlayerID(new ClientPlatformID(value));
        }
    ...
    protected override void OnMetadataChanged(string key, string value)
    {
        ...
        if (GetPartnerKey(PlayerIDManager.LocalID) == key)
        {
            ...
            {
                OnReceivePartner(PlayerIDManager.GetPlayerID(new ClientPlatformID(value)));
            }
        }
        else if (value != "-1" && key.StartsWith(PlayerPartnerKey))
        {
            ...
            if (id != null)
            {
                _tethers.Add(new EntangledTether(id, PlayerIDManager.GetPlayerID(new ClientPlatformID(value))));
            }
        }
    }
