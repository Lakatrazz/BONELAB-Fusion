...
namespace LabFusion.SDK.Points
{
    ...
    public sealed class PointShopPanelView : FusionPanelView
    {
        ...
        private int _catalogPageCount;
        private int _ownedPageCount;
        ...
        private void PushCatalogUpdate()
        {
            ...
            {
                _catalogPageCount = (int)Math.Ceiling(CatalogItems.Count / (double)_itemButtonCount);
            }
        }
        ...
        private void PushOwnedUpdate()
        {
            ...
            {
                _ownedPageCount = (int)Math.Ceiling(OwnedItems.Count / (double)_itemButtonCount);
            }
        }
        ...
        private void ConfirmUpgrade()
        {
            ...
            if (PointItemManager.TryUpgradeItem(_targetInfoItem))
            {
                ...
                FusionAudio.Play3D(transform.position, FusionContentLoader.PurchaseSuccess);
            }
            // Failure
            else
            {
                FusionAudio.Play3D(transform.position, FusionContentLoader.PurchaseFailure);
            }
        }
