...
internal class EOSLobbyManager
{
    ...
    internal EOSLobbyManager(ProductUserId localUserId)
    {
        _localUserId = localUserId ?? throw new ArgumentNullException(nameof(localUserId));
    }

    internal void CreateLobby(Action<EpicLobby> onComplete)
    {
    ...
    internal void DestroyLobby(Action onComplete)
    {
        if (CurrentLobby == null)
        {
            onComplete?.Invoke();
    ...
    internal void KickMember(string platformId, Action<bool> onComplete = null)
    {
        if (CurrentLobby == null)
        {
    ...
    internal ProductUserId GetLobbyOwner()
    {
