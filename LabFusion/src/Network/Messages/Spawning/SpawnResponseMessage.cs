    ...
    public class SpawnResponseMessage : FusionMessageHandler
    {
        ...
        public override void HandleMessage(byte[] bytes, bool isServerHandled = false)
        {
            if (!isServerHandled)
            {
                ...
                NullableMethodExtensions.PoolManager_Spawn(spawnable, data.serializedTransform.position.ToUnityVector3(), data.serializedTransform.rotation.ToUnityQuaternion(), null,
                    true, null, (Action<GameObject>)(go => { OnSpawnFinished(owner, barcode, syncId, go, path, hand); }));
            }
            else
        ...
        public static void OnSpawnFinished(byte owner, string barcode, ushort syncId, GameObject go, string spawnerPath = "_", Handedness hand = Handedness.UNDEFINED)
        {
            ...
            if (magazine)
            {
                ...
                NullableMethodExtensions.AudioPlayer_PlayAtPoint(ammoInventory.ammoReceiver.grabClips, ammoInventory.ammoReceiver.transform.position, null, null, false);
