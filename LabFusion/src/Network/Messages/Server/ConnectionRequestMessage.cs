...
public class ConnectionRequestData : INetSerializable
{
    ...
    public static ConnectionRequestData Create(string platformID, Version version, string avatarBarcode, SerializedAvatarStats stats)
    {
        LocalPlayer.InvokeApplyInitialMetadata();

        return new ConnectionRequestData()
        {
            BackupPlatformID = platformID,
            Version = version,
