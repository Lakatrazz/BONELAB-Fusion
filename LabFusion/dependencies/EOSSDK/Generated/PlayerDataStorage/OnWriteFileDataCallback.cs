...
namespace Epic.OnlineServices.PlayerDataStorage
{
	...
	/// </summary>
	/// <param name="data">
	/// Struct containing metadata for the file being written to, as well as the max length in bytes that can be safely written to DataBuffer
	/// </param>
	/// <param name="outDataBuffer">
	/// A buffer to write data into, to be appended to the end of the file that is being written to. The maximum length of this value is provided in the Info parameter. The number of bytes written to this buffer should be set in OutDataWritten.
	/// </param>
	/// <param name="outDataWritten">
	/// The length of the data written to OutDataBuffer. This must be less than or equal than the DataBufferLengthBytes provided in the Info parameter
	/// </param>
	/// <returns>
	...
	internal static class OnWriteFileDataCallbackInternalImplementation
	{
		...
		public static OnWriteFileDataCallbackInternal Delegate
		{
			get
			{
				if (s_Delegate == null)
				{
					s_Delegate = new OnWriteFileDataCallbackInternal(EntryPoint);
				}
		...
		public static WriteResult EntryPoint(ref WriteFileDataCallbackInfoInternal data, IntPtr outDataBuffer, out uint outDataWritten)
		{
			...
			{
				var outDataBufferPublic = default(ArraySegment<byte>);

				var callResult = callback(ref callbackInfo, out outDataBufferPublic);

				Helper.Get(outDataBufferPublic, out outDataWritten);
				Helper.Copy(outDataBufferPublic, outDataBuffer);

				return callResult;
			}

			outDataWritten = default;
			return default;
		}
	}
}
